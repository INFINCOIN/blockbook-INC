...
import (
	...
	"github.com/INFINCOIN/blockbook-INC/bchain"
	"github.com/INFINCOIN/blockbook-INC/bchain/coins/btc"
	"github.com/INFINCOIN/blockbook-INC/common"
)

// infincoinRPC is an interface to JSON-RPC bitcoind service
type infincoinRPC struct {
	*btc.BitcoinRPC
}
...
// NewinfincoinRPC returns new infincoinRPC instance
func NewinfincoinRPC(config json.RawMessage, pushHandler func(bchain.NotificationType)) (bchain.BlockChain, error) {
...
// Initialize initializes infincoinRPC instance
func (z *infincoinRPC) Initialize() error {
	...
	z.Parser = NewinfincoinParser(params, z.ChainConfig)
...
func (z *infincoinRPC) GetChainInfo() (*bchain.ChainInfo, error) {
...
func (z *infincoinRPC) GetBlock(hash string, height uint32) (*bchain.Block, error) {
	var err error
	if hash == "" && height > 0 {
...
func (z *infincoinRPC) GetTransactionForMempool(txid string) (*bchain.Tx, error) {
	return z.GetTransaction(txid)
}

// GetMempoolEntry returns mempool data for given transaction
func (z *infincoinRPC) GetMempoolEntry(txid string) (*bchain.MempoolEntry, error) {
